...
public class FileDAO{
    ...
    private static final String[] SEMANTIC_FIELDS={
        "fileName",
        "localPath",
        "fileSize",
        "fileType",
    ...
    public String saveFile(String uid,FileData file)throws Exception{
        ...
        if(fileId==null||fileId.isBlank())
            fileId=getUserFilesCollection(uid)
                    .document()
                    .getId();
        ...
        if(file.getFileName()!=null)
            data.put(
                    "fileNameLower",
                    file.getFileName().trim().toLowerCase()
            );
    ...
    public List<FileData> getFileSummariesBySpace(
            String uid,
            String spaceId
    )throws Exception{
    ...
    public List<FileData> searchFileSummaries(
            String uid,
            String searchText
    )throws Exception{
    ...
    public List<FileData> searchFilesForAI(
            String uid,
            String query
    )throws Exception{
        ...
        for(DocumentSnapshot document:
            ...
            try{
                ...
                System.out.println(
                        "[WARN] AI file search skipped file: "
                                +e.getMessage()
                );
            }
        }
    ...
    private int calculateAIRelevance(
            FileData file,
            String query
    ){
        ...
        List<String> terms=
                getKeywords(normalizedQuery);

        if(terms.isEmpty())
            return 0;

        String name=
                normalize(file.getFileName());

        String snippet=
                normalize(file.getExtractedSnippet());

        String description=
                normalize(file.getDescription());

        String category=
                normalize(file.getAiCategory());

        String tags=
                normalizeTags(file.getSmartTags());
    ...
    private int getDocumentTypeBonus(
            String query,
            String name,
            String snippet,
            String description
    ...
    public List<FileData> getFilesBySpace(
            String uid,
            String spaceId
    )throws Exception{
        return getFileSummariesBySpace(
                uid,
                spaceId
        );
    }
    ...
    public List<FileData> searchFiles(
        ...
        return searchFileSummaries(
                uid,
                fileName
        );
    }

    public boolean fileExistsByHash(
            String uid,
            String hash
    )throws Exception{
    ...
    public String getFileNameByHash(
            String uid,
            String hash
    )throws Exception{
    ...
    public FileData findFileByHash(
            String uid,
            String hash
    )throws Exception{
    ...
    public List<FileData> getTrashedFiles(
            String uid
    )throws Exception{
    ...
    private List<FileData> convertDocuments(
            QuerySnapshot snapshot
    ){
        ...
        for(DocumentSnapshot doc:
            ...
            try{
                ...
                file.setLastAccessedAt(
                        doc.getTimestamp(
                                "lastAccessedAt"
                        )
                );

                file.setExtractedSnippet(
                        doc.getString(
                                "extractedSnippet"
                        )
                );
                ...
                System.out.println(
                        "[WARN] Could not read file summary: "
                                +e.getMessage()
                );
            }
        }
    ...
    public List<FileData> getFilesForSemanticSearch(
            String uid
    )throws Exception{

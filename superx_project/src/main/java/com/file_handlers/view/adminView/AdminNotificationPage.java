...
public class AdminNotificationPage {
    ...
    private void applyHover(Node node, double scale, double dy) {
    ...
    private VBox createSidebar() {
        ...
        logoText.setFont(Font.font(FONT, FontWeight.BOLD, 19)); logoText.setTextFill(Color.web(WHITE));
        HBox logoRow = new HBox(10, createLogo(), logoText); logoRow.setAlignment(Pos.CENTER_LEFT);
        VBox logoSection = new VBox(4, logoRow); logoSection.setPadding(new Insets(0, 0, 18, 6));

        Button dashboardBtn = createSidebarButton("dashboard", "Dashboard", false); dashboardBtn.setOnAction(e -> LandingPage.showAdminDashboard());
        Button usersBtn = createSidebarButton("users", "Users", false); usersBtn.setOnAction(e -> LandingPage.showAdminUsers());
        Button filesBtn = createSidebarButton("files", "Files", false); filesBtn.setOnAction(e -> LandingPage.showAdminFiles());
        Button collabBtn = createSidebarButton("collaboration", "Collaboration", false); collabBtn.setOnAction(e -> LandingPage.showAdminCollaboration());
        Button aiBtn = createSidebarButton("ai", "AI System", false); aiBtn.setOnAction(e -> LandingPage.showAdminAISystem());
        Button analyticsBtn = createSidebarButton("analytics", "Analytics", false); analyticsBtn.setOnAction(e -> LandingPage.showAnalytics());
        Button securityBtn = createSidebarButton("security", "Security", false); securityBtn.setOnAction(e -> LandingPage.showAdminSecurity());

        VBox navigation = new VBox(4, dashboardBtn, usersBtn, filesBtn, collabBtn, aiBtn, analyticsBtn, securityBtn);
        Region spacer = new Region(); VBox.setVgrow(spacer, Priority.ALWAYS);

        Button settingsBtn = createSidebarButton("settings", "Settings", false); settingsBtn.setOnAction(e -> LandingPage.showAdminSettings());
        Region divider = new Region(); divider.setPrefHeight(1); divider.setStyle("-fx-background-color: " + SIDEBAR_BORDER + ";");
        Button logoutBtn = createSidebarButton("logout", "Logout", false); logoutBtn.setOnAction(event -> LandingPage.showAdminLoginPage());
    ...
    private StackPane createLogo() {
        ...
        StackPane logoPane = new StackPane(logoView); logoPane.setPrefSize(42, 42); logoPane.setAlignment(Pos.CENTER);
        applyHover(logoPane, 1.1, 0);
        return logoPane;
    }

    private Button createSidebarButton(String type, String text, boolean active) {
        SVGPath icon = createIcon(type);
        icon.setStroke(Color.web(active ? WHITE : LIGHT_SECONDARY)); icon.setStrokeWidth(2);
        StackPane iconBox = new StackPane(icon); iconBox.setPrefSize(24, 24);
        Label label = new Label(text);
        label.setFont(Font.font(FONT, active ? FontWeight.BOLD : FontWeight.MEDIUM, 13)); label.setTextFill(Color.web(WHITE));

        HBox row = new HBox(12, iconBox, label); row.setAlignment(Pos.CENTER_LEFT);
        Button button = new Button(); button.setGraphic(row);
        button.setPrefHeight(38); button.setMinHeight(38); button.setMaxWidth(Double.MAX_VALUE);
        ...
        if (active) {
            ...
            button.setOnMouseExited(e -> {
                button.setStyle("-fx-background-color: transparent; -fx-background-radius: 12; -fx-cursor: hand; -fx-border-width: 0;");
                icon.setStroke(Color.web(LIGHT_SECONDARY)); label.setTextFill(Color.web(WHITE));
                TranslateTransition tt = new TranslateTransition(Duration.millis(120), button); tt.setToX(0); tt.play();
            });
        }
        return button;
    }
    ...
    private HBox createTopBar() {
        ...
        HBox profile = new HBox(10, avatar, admin); profile.setAlignment(Pos.CENTER); profile.setPadding(new Insets(4, 12, 4, 6));
        ...
        applyHover(profile, 1.04, 0);

        Popup profilePopup = createProfilePopup();
        profile.setOnMouseClicked(e -> {
            if (profilePopup.isShowing()) profilePopup.hide();
            else {
        ...
        topBar.setAlignment(Pos.CENTER_RIGHT); topBar.setPrefHeight(70); topBar.setMinHeight(70); topBar.setMaxHeight(70);
    ...
    private HBox createProfilePopupItem(String iconType, String text, String iconColor, Runnable action) {
        SVGPath icon = createIcon(iconType); icon.setStroke(Color.web(iconColor)); icon.setStrokeWidth(2.0);
        StackPane iconBox = new StackPane(icon); iconBox.setPrefSize(22, 22);
        Label label = new Label(text); label.setFont(Font.font(FONT, FontWeight.NORMAL, 13)); label.setTextFill(Color.WHITE);
    ...
    private VBox createNotificationContent() {
        ...
        HBox headerRow = new HBox(titleBox, headerSpacer, markAllReadBtn); headerRow.setAlignment(Pos.CENTER_LEFT);

        HBox filterGroup = new HBox(8);
        Button pillAll = createFilterPill("All", true), pillReminders = createFilterPill("Reminders", false), pillCollab = createFilterPill("Collaboration", false);
        ...
        content.setFillWidth(true); content.setMaxWidth(Double.MAX_VALUE); content.setStyle("-fx-background-color: transparent;");
        return content;
    }
    ...
    private HBox createNotificationCard(NotificationItemData data) {
        ...
        row.setOnMouseEntered(e -> {
            ...
            TranslateTransition tt = new TranslateTransition(Duration.millis(120), row); tt.setToX(4); tt.play();
        });
        row.setOnMouseExited(e -> {
            row.setStyle(idleStyle);
            TranslateTransition tt = new TranslateTransition(Duration.millis(120), row); tt.setToX(0); tt.play();
        });
        return row;
    }
    ...
    private static class NotificationItemData {
        String iconType, title, subtitle, time, type;
        @SuppressWarnings("unused") boolean isRead = false;
        NotificationItemData(String iconType, String title, String subtitle, String time, String type) {
            this.iconType = iconType; this.title = title; this.subtitle = subtitle; this.time = time; this.type = type;
        }
    }
}
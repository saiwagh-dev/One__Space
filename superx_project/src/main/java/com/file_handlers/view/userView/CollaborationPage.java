...
public class CollaborationPage {
    ...
    private void initializeWorkspacesAndActivities() {
        ...
        try {
            ...
            for (com.google.cloud.firestore.DocumentSnapshot doc : workspacesDocs) {
                ...
                try {
                    var membersDocs = db.collection("workspaces").document(docId).collection("members").get().get()
                            .getDocuments();
                    ...
                    for (var mDoc : membersDocs) {
                        ...
                        if (mName != null) {
                            tempWorkspaceActivities
                                    .add(new ActivityItem(mName, "joined '" + spaceName + "'", "Recently"));
                        }
                    }
                ...
                try {
                    var filesDocs = db.collection("workspaces").document(docId).collection("files").get().get()
                            .getDocuments();
                    fileCount = filesDocs.size();
                    for (var fDoc : filesDocs) {
                        String fName = fDoc.getString("fileName");
                        if (fName != null) {
                            tempWorkspaceActivities.add(new ActivityItem("Team Member",
                                    "uploaded '" + fName + "' to " + spaceName, "Just now"));
                        }
                    }
                } catch (Exception ignored) {
                }
    ...
    public Scene getCollaborationPageScene() {
        ...
        topBar.setStyle("-fx-background-color: transparent; -fx-border-color: " + SIDEBAR_BORDER
        ...
        pendingBtn.setOnMouseEntered(e -> {
            pendingBtn.setStyle(
                    "-fx-background-color: rgba(56, 189, 248, 0.15); -fx-text-fill: #38BDF8; -fx-border-color: #38BDF8; -fx-border-radius: 8; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 8 14;");
        ...
        HBox stat1 = createStatCard("collaboration", spacesValue, "Total Shared Workspaces", "#38BDF8",
                "rgba(56, 189, 248, 0.15)");
        HBox stat2 = createStatCard("users", membersValue, "Members Across All Workspaces", "#A78BFA",
                "rgba(167, 139, 250, 0.15)");
        HBox stat3 = createStatCard("files", filesValue, "Files Across All Workspaces", "#34D399",
        ...
        toggleViewBtn.setOnMouseEntered(e -> {
            toggleViewBtn.setStyle(
                    "-fx-background-color: rgba(56, 189, 248, 0.15); -fx-text-fill: #38BDF8; -fx-border-color: #38BDF8; -fx-border-radius: 6; -fx-background-radius: 6; -fx-cursor: hand; -fx-padding: 5 10;");
            animateScale(toggleViewBtn, 1.04);
        });
        toggleViewBtn.setOnMouseExited(e -> {
            toggleViewBtn.setStyle("-fx-background-color: " + INPUT_BG + "; -fx-text-fill: " + WHITE
                    + "; -fx-border-color: " + INPUT_BORDER
                    + "; -fx-border-radius: 6; -fx-background-radius: 6; -fx-cursor: hand; -fx-padding: 5 10;");
        ...
        scrollPane.setStyle(
                "-fx-background-color: transparent; -fx-background: transparent; -fx-background-insets: 0; -fx-padding: 0;");
    ...
    private VBox createSidebar() {
        ...
        Button collabBtn = createSidebarButton("collaboration", "Collaboration", true,
        ...
        VBox navList = new VBox(4, dashboardBtn, spacesBtn, searchBtn, calendarBtn, aiBtn, collabBtn, recentBtn,
                trashBtn);
    ...
    private Button createSidebarButton(String iconType, String text, boolean active,
    ...
    private void updateMetrics() {
        ...
        if (membersValue != null)
            membersValue.setText(totalMembers + " Members");
        if (filesValue != null)
   ...
   private void rebuildWorkspaceCards(BorderPane root) {
    ...
    private HBox createWorkspaceCard(WorkspaceData w, BorderPane root, String docId) {
        ...
        iconPane.setStyle("-fx-background-color: " + w.badgeBg + "; -fx-background-radius: 50%; -fx-border-color: "
                + w.iconColor + "55; -fx-border-radius: 50%;");
        ...
        if ("Owner".equalsIgnoreCase(w.role) && root != null && docId != null && !docId.isEmpty()) {
            ...
            deleteBtn.setStyle(
                    "-fx-background-color: rgba(239, 68, 68, 0.12); -fx-background-radius: 6; -fx-padding: 4 6; -fx-cursor: hand;");

            deleteBtn.setOnMouseEntered(e -> deleteBtn.setStyle(
                    "-fx-background-color: rgba(239, 68, 68, 0.3); -fx-background-radius: 6; -fx-padding: 4 6; -fx-cursor: hand;"));
    ...
    private VBox createWorkspaceGridCard(WorkspaceData w, BorderPane root, String docId) {
        ...
        roleTag.setStyle("-fx-text-fill: " + w.badgeText + "; -fx-background-color: " + w.badgeBg
    ...
    private void deleteWorkspace(String docId, BorderPane root) {
        ...
        confirmDialog.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                ...
                new Thread(() -> {
                    try {
                        com.google.cloud.firestore.Firestore db = com.file_handlers.config.FirebaseConfig
                                .getFirestore();

                        // 1. Delete members subcollection documents
                        var members = db.collection("workspaces").document(docId).collection("members").get().get()
                                .getDocuments();
                        for (var m : members) {
    ...
    private void showManageMembersPopup(String spaceDocId) {
        ...
        try {
            ...
            for (var mDoc : membersDocs) {
                ...
                memberCard.setStyle(
                        "-fx-background-color: rgba(13, 22, 38, 0.9); -fx-border-color: rgba(255, 255, 255, 0.08); -fx-border-radius: 8; -fx-background-radius: 8;");
            ...
            for (com.google.cloud.firestore.DocumentSnapshot wsDoc : workspacesDocs) {
                ...
                foundAny = true;
                        ...
                        javafx.application.Platform.runLater(() -> {
                            list.getChildren()
                                    .add(pendingRequest(finalName, finalEmail, finalSpaceName, finalOwner, spaceDocId));
                        });
                    }
                }
            }
    ...
    private HBox pendingRequest(String name, String email, String space, String invitedBy, String spaceDocId) {
        ...
        avatar.setAlignment(Pos.CENTER);
        avatar.setStyle("-fx-font-family: " + FONT
                + "; -fx-font-size: 11px; -fx-font-weight: 700; -fx-background-color: rgba(37, 99, 235, 0.2); -fx-background-radius: 50%; -fx-text-fill: #60A5FA;");
    ...
    private HBox createStatCard(String iconType, Label value, String description,
        ...
        iconPane.setStyle("-fx-background-color: " + iconBackground + "; -fx-background-radius: 8; -fx-border-color: "
                + iconColor + "44; -fx-border-radius: 8;");
        ...
        card.setOnMouseEntered(e -> {
            card.setStyle("-fx-background-color: " + CARD_BG
                    + "; -fx-border-color: #38BDF8; -fx-border-width: 1.2; -fx-border-radius: 20; -fx-background-radius: 20; -fx-effect: dropshadow(three-pass-box, rgba(56,189,248,0.35), 20, 0, 0, 6);");
    ...
    private void showCreateSharedSpacePopup(BorderPane root) {
        ...
        nameField.setStyle("-fx-font-family: " + FONT + "; -fx-font-size: 13px; -fx-background-color: " + INPUT_BG
                + "; -fx-text-fill: " + WHITE + "; -fx-prompt-text-fill: " + LIGHT_SECONDARY + "; -fx-border-color: "
                + INPUT_BORDER + "; -fx-border-radius: 8; -fx-background-radius: 8;");
        ...
        membersField.setStyle("-fx-font-family: " + FONT + "; -fx-font-size: 13px; -fx-background-color: " + INPUT_BG
                + "; -fx-text-fill: " + WHITE + "; -fx-prompt-text-fill: " + LIGHT_SECONDARY + "; -fx-border-color: "
                + INPUT_BORDER + "; -fx-border-radius: 8; -fx-background-radius: 8;");
            ...
            if (button == create) {
                ...
                if (!spaceName.isEmpty()) {
                    ...
                    try {
                        ...
                        String ownerEmail = UserSession.getInstance() != null ? UserSession.getInstance().getEmail()
                                : "owner@app.com";
                        ...
                        db.collection("workspaces").document(docId)
                                .collection("members").document(ownerEmail.toLowerCase().replaceAll("[^a-z0-9]", "_"))
                                .set(ownerData);
                        ...
                        if (selectedFile[0] != null) {
                            ...
                            activitiesList.add(0, new ActivityItem(ownerName,
                                    "uploaded '" + selectedFile[0].getName() + "' to " + spaceName, "Just now"));
                        }

                        activitiesList.add(0,
                                new ActivityItem(ownerName, "created workspace '" + spaceName + "'", "Just now"));
    ...
    private SVGPath createIcon(String type) {
        ...
        switch (type) {
            ...
            case "users":
                icon.setContent(
                        "M8 11 A3 3 0 1 0 8 5 A3 3 0 0 0 8 11 Z M16 11 A3 3 0 1 0 16 5 A3 3 0 0 0 16 11 Z M2 20 C2 16 5 14 8 14 C11 14 14 16 14 20 M12 15 C14 14 17 14 19 15 C21 16 22 18 22 20");
                break;
            case "security":

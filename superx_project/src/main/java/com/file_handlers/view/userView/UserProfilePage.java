...
public class UserProfilePage {
    ...
    public Scene getUserProfilePageScene() {
        ...
        grid.setHgap(18); grid.setVgap(16);
        grid.add(nameBox, 0, 0); grid.add(emailBox, 1, 0);
        grid.add(usernameBox, 0, 1); grid.add(bioBox, 1, 1);
        ...
        VBox content = new VBox(20, pageHeader, profileSummary, detailsCard, accountCard, actionButtons);
    ...
    private VBox createSidebar() {
        ...
        HBox storageValues = new HBox(sidebarStorageVal, spacer(), sidebarStoragePercent);
        ...
        VBox storageCard = new VBox(8, storageTitle, storageValues, sidebarStorageProgress, manageStorage);
    ...
    private void loadProfileFromFirestore1() {
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                if (username == null || username.isBlank()) username = createDefaultUsername(session.getDisplayName());
                if (bio == null || bio.isBlank()) bio = DEFAULT_BIO;
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                if (!authUpdated) {
                    Platform.runLater(() -> {
                        saveStatus.setText("");
                        showAlert(Alert.AlertType.ERROR, "Profile Update Failed", "Unable to update display name.");
                    });
                    return;
                }
                ...
                Platform.runLater(() -> {
                    ...
                    showAlert(Alert.AlertType.INFORMATION, "Profile Updated", "Profile changes saved successfully.");
                });
            } catch (Exception e) {
                Platform.runLater(() -> {
                    saveStatus.setText("");
                    showAlert(Alert.AlertType.ERROR, "Profile Update Failed", "Unable to save profile.");
                });
            }
        });
        thread.setDaemon(true);
    ...
    private void showEditProfileDialog() {
        ...
        VBox content = new VBox(10, fieldBox("Full Name", name), fieldBox("Email Address", email), fieldBox("Username", username), fieldBox("Bio", bio));
        content.setPadding(new Insets(10)); content.setPrefWidth(380);
        ...
        dialog.setResultConverter(result -> {
            ...
            if (newName.isEmpty() || newUsername.isEmpty()) {
                showAlert(Alert.AlertType.WARNING, "Invalid Input", "Name and username cannot be empty.");
                return null;
            }
    ...
    private void chooseProfilePhoto() {
        ...
        try {
            ...
            imageView.setFitWidth(92); imageView.setFitHeight(92); imageView.setPreserveRatio(false);
    ...
    private String getEmail(UserSession session) {
        ...
        return (email == null || email.isBlank()) ? "No email available" : email.trim();
    }
    ...
    private String createDefaultUsername(String displayName) {
        ...
        return normalizeUsername(getFirstName(displayName).toLowerCase());
    }
    ...
    private Label createAvatar(String initials, double size) {
        ...
        avatar.setPrefSize(size, size); avatar.setMinSize(size, size); avatar.setMaxSize(size, size);
        avatar.setAlignment(Pos.CENTER);
        avatar.setFont(Font.font(FONT, FontWeight.BOLD, size >= 70 ? 24 : 12));
        avatar.setStyle("-fx-background-color: linear-gradient(to bottom right, #2563EB, #00D2FF); -fx-background-radius: 50%; -fx-text-fill: white;");
        return avatar;
    }
    ...
    private Button primaryButton(String text, double size) {
        ...
        button.setStyle("-fx-background-color: linear-gradient(to right, #1D4ED8, #2563EB); -fx-text-fill: white; -fx-background-radius: 9; -fx-border-color: rgba(96, 165, 250, 0.6); -fx-border-radius: 9; -fx-border-width: 1; -fx-cursor: hand; -fx-padding: 10 22;");
        return button;
    }
    ...
    private String cardStyle() {
        return "-fx-background-color: " + CARD_BG + "; -fx-border-color: " + CARD_BORDER + "; -fx-border-width: 1.2; -fx-border-radius: 16; -fx-background-radius: 16;";
    }

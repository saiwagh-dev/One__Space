...
import com.file_handlers.service.EmbeddingService;
...
public class UserSearch{
    ...
    public Scene getUserSearchScene(){
        ...
        if(UserSession.getInstance()!=null&&
                ...
                !UserSession.getInstance().getDisplayName().isBlank()){
        ...
        profileOption.setOnMouseClicked(e->{
            if(userDropdownPopup.isShowing()) userDropdownPopup.hide();
                ...
                userDropdownPopup.show(profileOption, profileOption.localToScreen(0, profileOption.getHeight()+6).getX(), profileOption.localToScreen(0, profileOption.getHeight()+6).getY());
            }
        });
        ...
        Label confidenceBadge=label("Semantic Search Ready",10,FontWeight.BOLD,"#34D399");
        ...
        HBox aiHeader=new HBox(label("✦ AI Search",15,FontWeight.BOLD,WHITE),new Region(),confidenceBadge);
        HBox.setHgrow(aiHeader.getChildren().get(1),Priority.ALWAYS);
        aiHeader.setAlignment(Pos.CENTER_LEFT);

        Label aiText=label("Search using the meaning of your files, not only exact keywords.",13,FontWeight.NORMAL,LIGHT_SECONDARY);
        ...
        aiCard.setStyle("-fx-background-color: "+CARD_BG+";-fx-border-color: "+CARD_BORDER+";-fx-border-width: 1.2;-fx-border-radius: 16;-fx-background-radius: 16;");
        ...
        gridContainer.setHgap(14); gridContainer.setVgap(14);
        ...
        new Thread(()->{
            try{
                ...
                if(query==null||query.isBlank()){
                    data=fileDAO.getFileSummaries(uid);
                }else{
                    data=semanticSearch(uid,query);
                }
    ...
    private List<FileData> semanticSearch(String uid,String query)throws Exception{
        List<Double> queryVector=embeddingService.embedQuery(query);
        if(queryVector==null||queryVector.isEmpty()){
            return fileDAO.searchFileSummaries(uid,query);
        }

        List<FileData> allFiles=fileDAO.getFilesForSemanticSearch(uid);
        List<FileMatch> matches=new ArrayList<>();

        for(FileData file:allFiles){
            List<Double> vector=file.getEmbedding();
            if(vector==null||vector.isEmpty()) continue;
            double score=cosineSimilarity(queryVector,vector);
            if(score>=0) matches.add(new FileMatch(file,score));
        }

        matches.sort((a,b)->Double.compare(b.score,a.score));
        List<FileData> result=new ArrayList<>();
        int limit=Math.min(20,matches.size());
        for(int i=0;i<limit;i++) result.add(matches.get(i).file);

        if(result.isEmpty()) return fileDAO.searchFileSummaries(uid,query);
        return result;
    }

    private double cosineSimilarity(List<Double> a,List<Double> b){
        if(a==null||b==null||a.isEmpty()||b.isEmpty()||a.size()!=b.size()) return -1;
        double dot=0, magnitudeA=0, magnitudeB=0;
        for(int i=0;i<a.size();i++){
            double x=a.get(i); double y=b.get(i);
            dot+=x*y; magnitudeA+=x*x; magnitudeB+=y*y;
        }
    ...
    private void renderList(){
        ...
        if(count==0){
            listContainer.getChildren().add(label(searchQuery.isBlank()?"No files found.":"No matching files.",13,FontWeight.NORMAL,LIGHT_SECONDARY));
        }
    ...
    private void renderGrid(){
        ...
        if(count==0){
            gridContainer.add(label(searchQuery.isBlank()?"No files found.":"No matching files.",13,FontWeight.NORMAL,LIGHT_SECONDARY),0,0);
        }
    ...
    private VBox createFileCard(FileInfo file,boolean isGrid){
        ...
        card.setStyle("-fx-background-color: "+CARD_BG+";-fx-border-color: "+CARD_BORDER+";-fx-border-width: 1.2;-fx-border-radius: 10;-fx-background-radius: 10;");

        if(isGrid) card.setPrefWidth(300);
        else card.setMaxWidth(Double.MAX_VALUE);

        card.setOnMouseEntered(e->{
            card.setStyle("-fx-background-color: "+CARD_BG+";-fx-border-color: #38BDF8;-fx-border-width: 1.2;-fx-border-radius: 10;-fx-background-radius: 10;");
    ...
    private VBox createSidebar(){
        ...
        HBox storageValGroup=new HBox(sidebarStorageVal,new Region(),sidebarStoragePercent);
    ...
    private String getExtension(String type,String name){
        if(type!=null&&!type.isBlank()&&!type.equalsIgnoreCase("OCTET-STREAM")){
    ...
    private static class FileMatch{
        final FileData file;
        final double score;
        FileMatch(FileData file,double score){
            this.file=file;
            this.score=score;
        }
    }

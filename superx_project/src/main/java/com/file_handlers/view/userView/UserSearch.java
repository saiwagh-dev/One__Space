...
public class UserSearch{
    ...
    public Scene getUserSearchScene(){
        ...
        dropdownSeparator.setStyle(
                "-fx-background-color: #1E293B;"+
                "-fx-padding: 4 0;"
        );

        VBox dropdownContainer=
                new VBox(
                        4,
                        profileDropdownBtn,
                        settingsDropdownBtn,
        ...
        userDropdownPopup.getContent().add(
                dropdownContainer
        );
        ...
        profileOption.setOnMouseClicked(e->{
            if(userDropdownPopup.isShowing())
                ...
                userDropdownPopup.show(
                        profileOption,
                        profileOption.localToScreen(
                                0,
                                profileOption.getHeight()+6
                        ).getX(),
                        profileOption.localToScreen(
                                0,
                                profileOption.getHeight()+6
        ...
        searchIcon.setStroke(
                Color.web(LIGHT_SECONDARY)
        );
        ...
        PauseTransition searchDelay=
                new PauseTransition(
                        Duration.millis(300)
                );

        mainSearchField.textProperty().addListener(
                (o,ov,nv)->{
                    ...
                    searchDelay.setOnFinished(
                            e->loadSearchResults(searchQuery)
                    );
        ...
        Label aiText=
                label(
                        "Search using the meaning of your files, not only exact keywords.",
                        13,
                        FontWeight.NORMAL,
                        LIGHT_SECONDARY
                );
    ...
    private List<FileData> semanticSearch(
            String uid,
            String query
    )throws Exception{
        ...
        if(queryVector==null||
            ...
            System.out.println(
                    "[SMART SEARCH] Query embedding failed."
            );

            return fileDAO.searchFileSummaries(
                    uid,
                    query
            );
        }
        ...
        List<FileData> allFiles=
                fileDAO.getFilesForSemanticSearch(
                        uid
                );
        ...
        if(!result.isEmpty()){
                FileMatch bestMatch=matches.get(0);
                System.out.println(
                        "[SMART SEARCH] \""+
                        query+
                        "\" → "+
                        bestMatch.file.getFileName()+
                        " | Confidence: "+
                        String.format(
                                "%.1f%%",
                                bestMatch.score*100
        ...
        if(result.isEmpty()){
            ...
            System.out.println(
                    "[SMART SEARCH] No embeddings found. "
                            +"Using normal search."
            );
    ...
    private VBox createFileCard(
        ...
        previewBtn.setOnAction(
                e->openFile(file.path)
        );

        folderBtn.setOnAction(
                e->openFolder(file.path)
        );
        ...
        card.setStyle(
                ...
                "-fx-border-radius: 10;"+
                "-fx-background-radius: 10;"+
                "-fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.6), 16, 0, 0, 6);"
        );
        ...
        card.setOnMouseEntered(e->{
            ...
            card.setStyle(
                    ...
                    "-fx-border-radius: 10;"+
                    "-fx-background-radius: 10;"+
                    "-fx-effect: dropshadow(three-pass-box, rgba(56,189,248,0.35), 16, 0, 0, 4);"
            );
    ...
    private Button createFileActionButton(
            String text
    ){
    ...
    private void openFile(String path){
        ...
        try{
            ...
            if(!f.exists()){
                ...
                showFileAlert(
                        "The file no longer exists at its stored location."
                );
    ...
    private void openFolder(String path){
        ...
        try{
            ...
            if(folder==null||!folder.exists()){
                ...
                showFileAlert(
                        "The local folder could not be found."
                );
            ...
            showFileAlert(
                    "Unable to open the local folder."
            );
        }
    }
    ...
    private Button createDropdownBtn(
            String text,
            String color,
            javafx.event.EventHandler<javafx.event.ActionEvent> act
    ){
    ...
    private VBox createSidebar(){
        ...
        HBox.setHgrow(
                storageValGroup.getChildren().get(1),
                Priority.ALWAYS
        );
        ...
        storageCard.setStyle(
                "-fx-background-color: rgba(16, 28, 48, 0.65);"+
                "-fx-border-color: "+SIDEBAR_BORDER+";"+
                "-fx-border-radius: 12;"+
                "-fx-background-radius: 12;"
        );
    ...
    private SVGPath createIcon(
        ...
        switch(type){
            ...
            case "files":
                icon.setContent(
                        "M5 2 H14 L19 7 V21 H5 Z M14 2 V7 H19 M8 11 H16 M8 15 H16 M8 18 H13"
                );
                break;

            case "search":
                icon.setContent(
                        "M10 3 A7 7 0 1 0 10 17 A7 7 0 0 0 10 3 Z M15 15 L21 21"
                );
                break;
            ...
            case "bell":
                icon.setContent(
                        "M6 17 H18 M8 17 V10 A4 4 0 0 1 16 10 V17 M10 20 H14"
                );
                break;
